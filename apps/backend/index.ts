...
app.post("/api/spells", async (req, res) => {
  try {
    ...
    if (!complexityLevel || complexityLevel < 1) {
      return res
        .status(400)
        .json({ error: "Valid complexity level is required (minimum 1)" });
    }
    if (!description) {
...
app.post("/api/spells/import", async (req, res) => {
  try {
    ...
    if (!Array.isArray(spells)) {
      return res
        .status(400)
        .json({ error: 'Request body must contain a "spells" array' });
    }
    ...
    const existingSpellNames = new Set(
      existingSpells.map((s) => s.name.toLowerCase()),
    );
    ...
    spells.forEach((spellData, index) => {
      ...
      if (
        !spellData.convocation ||
        !validConvocations.includes(spellData.convocation)
      ) {
        spellErrors.push(
          `convocation must be one of: ${validConvocations.join(", ")}`,
        );
      }

      if (
        !spellData.complexityLevel ||
        typeof spellData.complexityLevel !== "number" ||
        spellData.complexityLevel < 1
      ) {
      ...
      if (spellData.bonusEffects && !Array.isArray(spellData.bonusEffects)) {
        ...
        spellData.bonusEffects.forEach((effect: any, effectIndex: number) => {
          if (
            ...
            typeof effect.masteryLevelMinimum !== "number"
          ) {
            spellErrors.push(
              `bonusEffects[${effectIndex}].masteryLevelMinimum must be a number`,
            );
          }
          if (
            !effect.effectsDescription ||
            typeof effect.effectsDescription !== "string"
          ) {
            spellErrors.push(
              `bonusEffects[${effectIndex}].effectsDescription must be a string`,
            );
          }
        });
      }
      ...
      if (spellErrors.length > 0) {
        errors.push(
          `Spell ${index + 1} (${spellData.name || "unnamed"}): ${spellErrors.join(", ")}`,
        );
      } else {
...
app.put("/api/folders", async (req, res) => {
  try {
    ...
    if (!oldPath || !newPath) {
      return res
        .status(400)
        .json({ error: "Old and new folder paths are required" });
    }
    if (oldPath === "/") {
    ...
    if (oldPath === newPath) {
      return res
        .status(400)
        .json({ error: "New folder path must be different from old path" });
    }
...
app.patch("/api/folders/move", async (req, res) => {
  try {
    ...
    if (!sourcePath || !targetParentPath) {
      return res
        .status(400)
        .json({ error: "Both sourcePath and targetParentPath are required" });
    }
...
app.post("/api/characters", async (req, res) => {
  try {
    ...
    if (
      !convocations ||
      !Array.isArray(convocations) ||
      convocations.length === 0
    ) {
      return res
        .status(400)
        .json({ error: "At least one convocation is required" });
    }
    if (!rank) {
...
app.delete("/api/characters/:id/spells/:spellId", async (req, res) => {
  try {
    ...
    if (!character.knownSpellIds.includes(spellId)) {
      return res
        .status(404)
        .json({ error: "Character does not know this spell" });
    }
